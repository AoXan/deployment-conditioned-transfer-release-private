...
Source routes use knowledge distillation (KD). Predictive performance is reported as mean absolute error (MAE), root mean squared error (RMSE), and coefficient of determination ($R^2$), subject to the metrics available for each study.
...
Roseworthy E5 & Within-paddock point-level yield; complete-input and soil-unavailable spatial settings with scratch and supervised source-initialised routes. & Relates transfer value to deployment-time modality availability at within-paddock scale.\\
...
\centering
\small
\caption{Roseworthy E5 within-paddock results. ``Sup. init.'' denotes supervised source-initialised fine-tuning. The upstream source dataset is not specified in the study documentation.}\label{tab:roseworthy-results}
\setlength{\tabcolsep}{3pt}
...
\begin{figure}[tb]
\centering
\includegraphics[width=.96\textwidth]{../figures/final_publication_v4_18/supplement_figure_s2.pdf}
\caption{Signed feature-level Permutation-SHAP redistribution relative to supervised transfer. GROUP and SPATIAL use the same feature order and colour scale; values are sample-cluster means. The vertical divider separates observation-support from physical-content features.}
...
Roseworthy E5 is a point-level, within-paddock yield setting with 43,311 registered observations from 2007--2008. Specific public repository identifiers are withheld during anonymous review. The exact correspondence between the cleaned analysis file and the public repository records remains unresolved, so we describe the within-paddock comparison without assigning a public source record. The spatial comparison uses weather and soil for complete-input evaluation, then removes soil at deployment. Under complete inputs, scratch gives MAE 0.465, RMSE 0.637, and $R^2$ 0.131; supervised source-initialised fine-tuning gives 0.465, 0.637, and 0.133. Under no-soil deployment, scratch gives 0.499, 0.667, and 0.049, whereas the transferred model gives 0.465, 0.637, and 0.133. Prediction, representation, combined, and missing-aware routes are listed in Table~\ref{tab:roseworthy-results}. The transferred model remains stable when soil is removed, whereas the local scratch model degrades.

The upstream source dataset, crop, and checkpoint are not specified. We therefore interpret this result as a within-paddock modality comparison rather than attributing it to a particular cross-crop source.
...
\input{generated_tables/apsim_input_conditions}
...
\input{generated_tables/apsim_common_comparison}
...
\input{generated_tables/apsim_paired_differences}
...
\input{generated_tables/apsim_sensitivity}
